...
export default {
  ...
  onLoad(options) {
    ...
    this.headerTop =
      pageHead && pageHead[0] ? pageHead.offsetHeight + "px" : "0px";
    // #endif
  ...
  methods: {
    ...
    loadData(type = "add", loading) {
      ...
      that.$http
        .post(that.$api.goods.gallery, postData)
        .then((res) => {
          console.log(res);
          if (type === "refresh") {
            that.goodsList = [];
          ...
          that.loadingType =
            res.data.page >= res.data.pagetotal ? "nomore" : "more";
